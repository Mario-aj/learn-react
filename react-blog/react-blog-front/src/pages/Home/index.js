import {
  Container,
  Post,
  Header,
  Title,
  Author,
  Image,
  Description,
} from './styles';

const Home = () => {
  return (
    <Container>
      <Post>
        <Header>
          <Title>NWL Together</Title>
          <Author>@Author: Mario Jorge</Author>
        </Header>
        <Image
          src="https://nextlevelweek.com/og/nlw-edition.png"
          alt="bannerUrl"
        />

        <Description>
          Um evento para dar o próximo passo na sua evolução como programadora
          ou programador. Um evento para dar o próximo passo na sua evolução
          como programadora ou programador. Um evento para dar o próximo passo
          na sua evolução como programadora ou programador. Um evento para dar o
          próximo passo na sua evolução como programadora ou programador.
        </Description>
      </Post>

      <Post>
        <Header>
          <Title>Learning Reactjs</Title>
          <Author>@Author: Luis Rosada</Author>
        </Header>
        <Image
          src="data:image/png;base64,"
          alt="bannerUrl"
        />

        <Description>
          Um evento para dar o próximo passo na sua evolução como programadora
          ou programador. Um evento para dar o próximo passo na sua evolução
          como programadora ou programador. Um evento para dar o próximo passo
          na sua evolução como programadora ou programador. Um evento para dar o
          próximo passo na sua evolução como programadora ou programador.
        </Description>
      </Post>

      <Post>
        <Header>
          <Title>Build Real-Time API in Nodejs</Title>
          <Author>@Author: Diego Fernandes</Author>
        </Header>
        <Image
          src="https://miro.medium.com/max/2625/1*fQyEyu_9upvynA4REV3bsw.png"
          alt="bannerUrl"
        />

        <Description>
          Um evento para dar o próximo passo na sua evolução como programadora
          ou programador.Um evento para dar o próximo passo na sua evolução como
          programadora ou programador. Um evento para dar o próximo passo na sua
          evolução como programadora ou programador. Um evento para dar o
          próximo passo na sua evolução como programadora ou programador.
        </Description>
      </Post>
    </Container>
  );
};

export default Home;
